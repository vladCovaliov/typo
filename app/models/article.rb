...
class Article < Content
  ...
  def merge_with(id)
    merge_with =  Article.find(id) rescue ""
    if merge_with.present? && self.present?
      self.update_attributes(body: self.body + " " + merge_with.body)
      self.comments << merge_with.comments
      merge_with.delete
      self
    else
      false
